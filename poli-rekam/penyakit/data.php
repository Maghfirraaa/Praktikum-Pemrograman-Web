				...
				<tbody>
					<?php
						...
						if($_SERVER['REQUEST_METHOD'] == "POST"){
							...
							if($pencarian != ''){
								$sql = "SELECT * FROM tb_penyakit WHERE nm_penyakit LIKE '%$pencarian%'";
								$query = $sql;
								$query_jml = $sql;
							}
							else{
								$query = "SELECT * FROM tb_penyakit LIMIT $posisi, $batas";
								$query_jml = "SELECT * FROM tb_penyakit";
								$no = $posisi + 1;
							}
						}
						else{
						...
						$sql_penyakit = mysqli_query($con, $query) or die(mysqli_error($con));
							...
							while($data = mysqli_fetch_array($sql_penyakit)){ ?>
								<tr>
									...
									<td class="text-center">
										...
										<a href="del.php?id=<?= $data['id_penyakit'] ?>" onclick="return confirm('Yakin akan menghapus data?')" class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-trash"></i></a>
									</td>
								</tr>
							<?php
							}
